...
export const DoctorManagement = ({ onViewPatients }: { onViewPatients?: (doctorId: string) => void }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (response.ok) { setForm({ fullName: '', email: '', phoneNumber: '', password: '' }); void load(); }
  };
  ...
  return <section className="user-form-card mb-6 rounded-2xl border border-white bg-white/80 p-5 shadow-sm sm:p-6">
    ...
    <form onSubmit={submit} className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <input required value={form.fullName} onChange={e => setForm(v => ({ ...v, fullName: e.target.value }))} placeholder="Doctor full name" className="rounded-xl border border-slate-300 px-4 py-3 text-sm" />
      <input required type="email" value={form.email} onChange={e => setForm(v => ({ ...v, email: e.target.value }))} placeholder="doctor@example.com" className="rounded-xl border border-slate-300 px-4 py-3 text-sm" />
      <input required inputMode="numeric" maxLength={10} value={form.phoneNumber} onChange={e => setForm(v => ({ ...v, phoneNumber: e.target.value.replace(/\D/g, '').slice(0, 10) }))} placeholder="10-digit phone" className="rounded-xl border border-slate-300 px-4 py-3 text-sm" />
      <div className="flex gap-2"><input required type="password" minLength={8} value={form.password} onChange={e => setForm(v => ({ ...v, password: e.target.value }))} placeholder="Temporary password" className="min-w-0 flex-1 rounded-xl border border-slate-300 px-4 py-3 text-sm" /><button disabled={saving} className="rounded-xl bg-blue-600 px-4 text-sm font-bold text-white disabled:opacity-50">{saving ? '…' : 'Add'}</button></div>
    </form>
