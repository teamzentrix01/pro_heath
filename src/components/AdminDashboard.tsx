...
const sectionTitles: Record<AdminSection, { title: string; subtitle: string }> =
  {
    dashboard: {
      title: "Dashboard",
      subtitle: "Operational summary and referral totals",
    },
    accounts: {
      title: "PRO Accounts",
      subtitle: "Create and manage care network accounts",
    },
    analytics: {
      title: "Analytics",
      subtitle: "Review lead performance by PRO",
    },
    submissions: {
      title: "Submitted Leads",
      subtitle: "Review patients, treatment, and payments",
    },
  };
...
export const AdminDashboard = () => {
  ...
  const [submissionPeriodFilter, setSubmissionPeriodFilter] = useState<
    "All" | "weekly" | "monthly"
  >("All");
  const [submissionPeriodDate, setSubmissionPeriodDate] = useState(() =>
  ...
  const activeLead = reviewLead
    ? submissions.find((s) => s.id === reviewLead.id) || reviewLead
    : null;
  ...
  const [analyticsGroupBy, setAnalyticsGroupBy] = useState<"pro" | "doctor">(
    "pro",
  );
  ...
  const [userAnalyticsRows, setUserAnalyticsRows] = useState<
    UserAnalyticsRow[]
  >([]);
  const [expandedAnalyticsRows, setExpandedAnalyticsRows] = useState<
  ...
  const loadSubmissions = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (silent && knownSubmissionIdsRef.current.size > 0) {
        const newCount = incomingSubmissions.filter(
          (item) => !knownSubmissionIdsRef.current.has(item.id),
        ).length;
        if (newCount > 0) setNewLeadNotice(newCount);
      }
      knownSubmissionIdsRef.current = new Set(
        incomingSubmissions.map((item) => item.id),
      );
  ...
  useEffect(() => {
    ...
    const loadUserAnalytics = async () => {
      ...
      try {
        ...
        if (analyticsPatientName)
          params.append("patientName", analyticsPatientName);
        ...
        setAnalyticsError(
          "Analytics could not be loaded. Please try changing a filter or refresh the page.",
        );
      } finally {
        if (!controller.signal.aborted) setIsAnalyticsLoading(false);
      }
    };
    ...
    try {
      ...
      setDashboardError(
        error instanceof Error
          ? error.message
          : "Unable to update this lead status.",
      );
    } finally {
    ...
    setSubmissions((current) =>
      current.map((item) =>
        item.id === submissionId
          ? {
              ...item,
              adminSeenAt: item.adminSeenAt ?? new Date().toISOString(),
            }
          : item,
      ),
    );
    await apiFetch(`/api/submissions/${submissionId}/seen`, {
      method: "PATCH",
    });
  };
  ...
  const markAllSeen = async () => {
    ...
    setSubmissions((current) =>
      current.map((item) => ({
        ...item,
        adminSeenAt: item.adminSeenAt ?? seenAt,
      })),
    );
  ...
  const totalDocuments = submissions.reduce(
    (sum, submission) => sum + submission.documents.length,
    0,
  );
  const totalReferralAmount = submissions.reduce(
    (sum, submission) => sum + (submission.referralAmount ?? 0),
    0,
  );
  const paidReferralAmount = submissions
    .filter((submission) => submission.paymentStatus === "Paid")
    .reduce((sum, submission) => sum + (submission.referralAmount ?? 0), 0);
  const unreadCount = submissions.filter(
    (submission) => !submission.adminSeenAt,
  ).length;
  const analyticsSummary = useMemo(() => {
    const totals = userAnalyticsRows.reduce(
      (summary, row) => ({
        leads: summary.leads + row.count,
        ...
        referralAmount:
          summary.referralAmount +
          row.leadsDetail.reduce(
            (sum, lead) => sum + (lead.referralAmount ?? 0),
            0,
          ),
      }),
      { leads: 0, pending: 0, approved: 0, rejected: 0, referralAmount: 0 },
    );
    ...
    return {
      ...
      approvalRate:
        totals.leads > 0
          ? Math.round((totals.approved / totals.leads) * 100)
          : 0,
    };
  }, [userAnalyticsRows]);

  const analyticsChartRows = useMemo(
    () => [...userAnalyticsRows].sort((a, b) => b.count - a.count).slice(0, 8),
    [userAnalyticsRows],
  );
  const analyticsChartMax = analyticsChartRows[0]?.count || 1;
  ...
  return (
    <div className="app-surface admin-dashboard">
        ...
        <div className="mx-auto flex max-w-[1600px] flex-row items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={
                unreadCount > 0
                  ? `${unreadCount} new lead notifications`
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -right-1.5 -top-1.5 grid min-h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white">
                  {unreadCount}
                </span>
              )}
            </button>
            <button
              type="button"
              onClick={() => setPasswordDialog({ mode: "change" })}
      ...
      <main className="admin-content space-y-8">
        ...
        <section
          id="dashboard"
          className={`${activeSection === "dashboard" ? "" : "hidden"} metric-grid grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5`}
        >
          ...
          <div className="bg-white rounded-lg shadow p-5">
            ...
            <p className="mt-2 text-3xl font-bold text-indigo-600">
              ₹{totalReferralAmount.toLocaleString("en-IN")}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow p-5">
            <p className="text-gray-600 text-sm font-medium">Referral Paid</p>
            <p className="mt-2 text-3xl font-bold text-emerald-600">
              ₹{paidReferralAmount.toLocaleString("en-IN")}
            </p>
          </div>
        </section>
        ...
        <section
          id="pros"
          className={`${activeSection === "accounts" ? "" : "hidden"} max-w-3xl`}
        >
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <div className="border-b border-gray-200 bg-white px-6 py-4">
              <h2 className="text-xl font-bold text-gray-900">
                Create PRO Credentials
              </h2>
            ...
            <form
              key={userFormResetKey}
              onSubmit={handleCreateUser}
              autoComplete="off"
              className="grid grid-cols-1 gap-5 p-6 xl:grid-cols-2"
            >
              {userCreationMessage && (
        ...
        <section
          className={`${activeSection === "accounts" ? "" : "hidden"} bg-white rounded-lg shadow overflow-hidden`}
        >
          <div className="px-6 py-5 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Account directory with assigned roles.
            </p>
          </div>
        ...
        <section
          className={`${activeSection === "submissions" ? "" : "hidden"} bg-white rounded-lg shadow p-6`}
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Filters</h2>
              <p className="text-sm text-gray-600 mt-1">
                Search by date, patient, doctor, PRO, location, status, or
                document name.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-5">
            ...
            <div>
              <label
                htmlFor="submissionPeriodFilter"
                className="block text-sm font-medium text-gray-700 mb-2"
              >
                Period
              </label>
              <select
                ...
                onChange={(event) =>
                  setSubmissionPeriodFilter(
                    event.target.value as "All" | "weekly" | "monthly",
                  )
                }
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  setSubmissionPeriodDate(event.target.value)
                }
        ...
        <section
          id="analytics"
          className={`${activeSection === "analytics" ? "" : "hidden"} bg-white rounded-2xl shadow-xl p-6 border border-slate-100`}
        >
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 bg-slate-50/70 p-5 rounded-2xl border border-slate-200/60 shadow-sm">
            <div>
              ...
              <select
                ...
                onChange={(event) =>
                  setAnalyticsPeriod(event.target.value as "weekly" | "monthly")
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  setAnalyticsPatientName(event.target.value)
                }
                className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-xs outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition shadow-sm font-medium"
              />
            </div>
          ...
          {isAnalyticsLoading ? (
            <div
              className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(16rem,0.7fr)]"
              aria-live="polite"
            >
            ...
            <div
              role="alert"
              className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-medium text-rose-700"
            >
              {analyticsError}
            </div>
          ) : userAnalyticsRows.length > 0 ? (
            <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(16rem,0.7fr)]">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-lg font-bold text-slate-900">
                      Status mix by{" "}
                      {analyticsGroupBy === "pro" ? "PRO" : "doctor"}
                    </h3>
                    <p className="mt-1 text-xs text-slate-500">
                      {formatAnalyticsRange(analyticsPeriod, analyticsDate)}
                    </p>
                  </div>
                  <div
                ...
                <div
                  ...
                  role="img"
                  aria-label={`Stacked bar chart comparing lead statuses for ${analyticsChartRows.length} ${analyticsGroupBy === "pro" ? "PRO accounts" : "doctors"}`}
                >
                  {analyticsChartRows.map((row) => {
                    const denominator = row.count || 1;
                    const rowKey = row.userId ?? row.email;
                    return (
                      <div
                        key={rowKey}
                        className="grid grid-cols-[minmax(6.5rem,0.8fr)_minmax(9rem,2fr)_2rem] items-center gap-3"
                      >
                        ...
                        <div
                          className="h-7 overflow-hidden rounded-lg bg-slate-100 ring-1 ring-inset ring-slate-200"
                          title={`${row.count} leads: ${row.pendingCount} pending, ${row.approvedCount} approved, ${row.rejectedCount} rejected`}
                        >
                          <div
                            className="flex h-full min-w-1 overflow-hidden rounded-lg"
                            style={{
                              width: `${(row.count / analyticsChartMax) * 100}%`,
                            }}
                          >
                            {row.pendingCount > 0 && (
                              <span
                                ...
                                style={{
                                  width: `${(row.pendingCount / denominator) * 100}%`,
                                }}
                              />
                            )}
                            {row.approvedCount > 0 && (
                              <span
                                ...
                                style={{
                                  width: `${(row.approvedCount / denominator) * 100}%`,
                                }}
                              />
                            )}
                            {row.rejectedCount > 0 && (
                              <span
                                ...
                                style={{
                                  width: `${(row.rejectedCount / denominator) * 100}%`,
                                }}
                              />
                            )}
                          </div>
                        </div>
                        <span className="text-right text-xs font-extrabold tabular-nums text-slate-700">
                          {row.count}
                        </span>
                      </div>
                    );
                ...
                {userAnalyticsRows.length > analyticsChartRows.length && (
                  <p className="mt-5 border-t border-slate-100 pt-3 text-[11px] text-slate-500">
                    Showing the top {analyticsChartRows.length} of{" "}
                    {userAnalyticsRows.length} results. The full list is below.
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {userAnalyticsRows.length} active{" "}
                    {analyticsGroupBy === "pro" ? "PROs" : "doctors"}
                  </p>
                </div>
                <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
                  ...
                  <p className="mt-3 text-3xl font-black tracking-tight text-slate-950">
                    {analyticsSummary.approvalRate}%
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    {analyticsSummary.approved} approved
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-violet-100 bg-violet-50/70 p-4">
                  ...
                  <p className="mt-3 text-2xl font-black tracking-tight text-slate-950">
                    ₹{analyticsSummary.referralAmount.toLocaleString("en-IN")}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    Across filtered leads
                  </p>
                </div>
              </div>
            </div>
            ...
            userAnalyticsRows.length === 0 ? (
              <div className="p-8 text-center text-slate-500 bg-slate-50/20">
                <p className="text-sm font-medium">
                  No leads match the selected filters and period.
                </p>
              </div>
            ) : userAnalyticsRows.length > 0 ? (
                      ...
                      <React.Fragment key={rowKey}>
                        ...
                        {isExpanded && (
                          <tr>
                            <td
                              colSpan={6}
                              className="bg-slate-50/50 px-6 py-4 border-t border-b border-slate-200/50"
                            >
                              <div className="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
                                ...
                                {row.leadsDetail.length === 0 ? (
                                  <p className="text-xs text-slate-500">
                                    No individual patient details found.
                                  </p>
                                ) : (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                    {row.leadsDetail.map((lead, idx) => (
                                      <div
                                        key={idx}
                                        className="flex flex-col p-3 rounded-xl border border-slate-100 bg-slate-50/40 hover:bg-slate-50 transition duration-150"
                                      >
                                        ...
                                        {lead.referralAmount !== null && (
                                          <div className="mt-2 text-xs font-bold text-emerald-600">
                                            Referral: ₹
                                            {lead.referralAmount.toLocaleString(
                                              "en-IN",
                                            )}
                                          </div>
                                        )}
        ...
        <section
          id="submissions"
          className={`${activeSection === "submissions" ? "" : "hidden"} bg-white rounded-lg shadow overflow-hidden`}
        >
                              ...
                              {submission.referralAmount !== null && (
                                <div>
                                  ...
                                  <span className="font-bold text-emerald-600">
                                    ₹
                                    {submission.referralAmount.toLocaleString(
                                      "en-IN",
                                    )}
                                  </span>
                                </div>
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (!submission.adminSeenAt)
                                void markSeen(submission.id);
                            }}
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                  ...
                  <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <dd className="text-gray-900 font-semibold">
                        {activeLead.contactNumber || "Not provided"}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-gray-500">Disease / Diagnosis</dt>
                      <dd className="text-gray-900 font-semibold whitespace-pre-line">
                        {activeLead.disease || "Not provided"}
                      </dd>
                    </div>
                    <div className="sm:col-span-2">
                      <dt className="text-gray-500">Permanent Address</dt>
                      <dd className="text-gray-900 font-semibold whitespace-pre-wrap">
                        {activeLead.address}
                      </dd>
                    </div>
                    <div className="sm:col-span-2">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                  ...
                  <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <dd className="text-gray-900 font-semibold">
                        {activeLead.submittedByName} (
                        {activeLead.submittedByRole.toUpperCase()})
                      </dd>
                    </div>
                    <div>
                      <dt className="text-gray-500">Email Address</dt>
                      <dd className="text-gray-900 font-semibold break-all">
                        {activeLead.submittedByEmail}
                      </dd>
                    </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                  ...
                  {activeLead.documents.length === 0 ? (
                    <p className="text-sm text-gray-500">
                      No files were uploaded with this lead.
                    </p>
                  ) : (
                ...
                {activeLead.status === "Pending" && (
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                    ...
                    <p className="text-xs text-slate-500 mb-4">
                      Please review the details and files. Approve or Reject
                      this referral.
                    </p>
                ...
                {activeLead.status === "Rejected" && (
                  <div className="rounded-xl border border-red-200 bg-red-50 p-4">
                    ...
                    <p className="mt-1 text-xs text-red-900 whitespace-pre-wrap leading-relaxed">
                      ...
                      {activeLead.rejectionReason || "No reason provided."}
                    </p>
                  </div>
                )}
      ...
      {statusAction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
          <section className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <span
              className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${statusAction.status === "Approved" ? "bg-emerald-100 text-emerald-700" : statusAction.status === "Rejected" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
            >
              {`${statusAction.status} lead`}
            </span>
            <h2 className="mt-4 text-xl font-bold text-slate-950">
              {statusAction.submission.fullName}
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              {statusAction.status === "Approved"
                ? "Confirm that this lead is ready to be approved. The decision cannot be reopened."
                : "Explain clearly why this lead is being rejected. This message will be visible to the PRO and doctor."}
            </p>
            {statusAction.status === "Rejected" && (
              <div className="mt-5">
                ...
                <textarea
                  ...
                  rows={4}
                  maxLength={500}
                  placeholder="Example: Required medical document is missing…"
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <button
                ...
                onClick={() =>
                  void updateStatus(
                    statusAction.submission.id,
                    statusAction.status,
                    statusReason.trim(),
                  )
                }
                className={`rounded-xl px-4 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40 ${statusAction.status === "Rejected" ? "bg-red-600 hover:bg-red-700" : "bg-emerald-600 hover:bg-emerald-700"}`}
              >
      ...
      {passwordDialog && (
        <PasswordDialog
          ...
          targetId={passwordDialog.target?.id}
          targetName={
            passwordDialog.target?.fullName ||
            user?.fullName ||
            user?.email ||
            "Admin"
          }
      ...
      {newLeadNotice > 0 && (
        <div className="fixed bottom-5 right-5 z-50 flex max-w-sm items-start gap-3 rounded-2xl border border-blue-200 bg-white p-4 shadow-2xl">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-600 text-lg text-white">
            +
          </span>
          <button
            type="button"
          ...
          >
            <strong className="block text-sm text-slate-950">
              {newLeadNotice} new lead{newLeadNotice > 1 ? "s" : ""} received
            </strong>
            <span className="mt-1 block text-xs text-slate-500">
              Tap to review the latest submission.
            </span>
          </button>
          <button
            type="button"
            onClick={() => setNewLeadNotice(0)}
